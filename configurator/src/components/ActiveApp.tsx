...
// created, and keeps it from then on regardless of prop changes underneath —
// re-rendering with new `params` doesn't reset it. A param that changes on
// the device (GetChangedAppParams — see ParamStore::update) needs a fresh
// useForm() instance to show up. `ActiveApp` below keys this component by
// the params' content, so it remounts exactly when they change — but stays
// separate from the `<details>` card around it, so opening/closing that card
// (native DOM state, not React state) survives the remount instead of
// getting reset every time a device-driven value comes in.
const ActiveAppParamsForm = ({
  app,
  layoutId,
  params,
