//! Persistent-storage plumbing: a request/response channel architecture with
//! static buffer pools, plus the length/checksum framing. The actual medium is
//! provided through [`StorageBackend`] — the firmware plugs in the FM24V10
//! FRAM over I2C, the simulator a file- or memory-backed buffer.
...
/// Backing medium for the storage service: reads and writes raw bytes at
/// absolute addresses. Implementations do not deal with framing — the
/// length/checksum header is handled here.
...
impl Drop for ReadGuard {
    fn drop(&mut self) {
        ...
        if AVAILABLE_READ_BUFFER_INDICES.try_send(self.index).is_err() {
            error!("CRITICAL: Failed to release FRAM buffer lease. A buffer is now lost and a deadlock may occur.");
        }
    }
}
...
pub async fn read_data(address: u32) -> Result<ReadGuard, FramError> {
    ...
    .await
    {
        Ok(idx) => idx,
        Err(_) => {
            warn!("Timed out acquiring a read buffer lease.");
    ...
    F: FnOnce(&mut [u8]) -> Result<usize, postcard::Error>,
{
    ...
    // `run_storage_service` is now responsible for releasing the token.
    Ok(())
}
...
struct Storage<B: StorageBackend> {
    backend: B,
...
/// Services all storage reads/writes over the given backend. Never returns;
/// run it as (part of) a dedicated task in the host binary.
pub async fn run_storage_service<B: StorageBackend>(backend: B) -> ! {
    ...
    loop {
        match select(FRAM_REQUEST_CHANNEL.receive(), WRITE_CHANNEL.receive()).await {
            ...
            Either::Second(request) => {
                ...
                if let Err(e) = result {
                    error!("FRAM write/erase failed: {:?}", e);
                }
