...
//!
//! NOTE: Fork from within embassy-usb until the new version is released upstream
...
use embassy_usb::{Builder, FunctionBuilder, Handler, InterfaceAltBuilder};
...
/// Helper API to expose func and alt in closure
pub struct WebUsbScopes<'f, 'd, D: Driver<'d>> {
    config: &'d Config<'d>,
    func: FunctionBuilder<'f, 'd, D>,
}

impl<'f, 'd, D: Driver<'d>> WebUsbScopes<'f, 'd, D> {
    /// Provides access to the FunctionBuilder
    /// E.g. to create extra msos features
    pub fn with_function<FN>(&mut self, f: FN) -> &mut Self
    where
        FN: for<'x> FnOnce(&mut FunctionBuilder<'x, 'd, D>),
    {
        f(&mut self.func);
        self
    }

    /// Provides access to the InterfaceAltBuilder
    /// Also creates an interface and applies relevant BOS capabilites
    /// Do your endpoint setup here and return the endpoints
    pub fn with_alt<T, FN>(&mut self, str_idx: Option<StringIndex>, f: FN) -> T
    where
        FN: for<'x> FnOnce(&mut InterfaceAltBuilder<'x, 'd, D>) -> T,
    {
        let mut iface = self.func.interface();
        ...
        alt.bos_capability(
            ...
            &[
                ...
                0x01,
                // bVendorCode
                self.config.vendor_code,
                // iLandingPage
                if self.config.landing_url.is_some() {
                    1
                } else {
                    0
                },
...
impl<'d, D: Driver<'d>> WebUsb<'d, D> {
    ...
    /// Then use the with_scopes closure to set up your endpoints
    pub fn configure_with<F, T>(
    ...
    where
        F: for<'f> FnOnce(&mut WebUsbScopes<'f, 'd, D>) -> T,
    {
        let ret = {
            let func = builder.function(USB_CLASS_VENDOR, USB_SUBCLASS_NONE, USB_PROTOCOL_NONE);
            let mut scopes = WebUsbScopes { config, func };
            with_scopes(&mut scopes)
        };
