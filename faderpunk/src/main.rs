...
async fn main(spawner: Spawner) {
    ...
    // Read before anything else can disturb the reason register or scratch0.
    // `Some` means the previous boot ended with an FPApp wedging Core 1, and
    // names the slot that was running when it did.
    let mut hw_watchdog = Watchdog::new(p.WATCHDOG);
    ...
    // Hold back any slot already quarantined, plus the one that just hung us.
    // RAM before FRAM deliberately: if persisting fails or wedges, this boot is
    // still protected, and scratch0 still names the slot for the next attempt.
    let mut quarantined = state::quarantined_slots().await;
    if let Some(slot) = timed_out_slot {
        defmt::warn!("FPApp slot {} hung the device; quarantining it", slot);
        quarantined |= 1 << slot;
    }
    fpapps::apply_quarantine(quarantined).await;
    if timed_out_slot.is_some() {
        state::update_state(|state| {
            let changed = state.quarantined_slots != quarantined;
            state.quarantined_slots = quarantined;
            changed
        })
        .await;
        // Safe to forget only now that the quarantine outlives the marker.
        hw_watchdog.set_scratch(0, 0);
    }
