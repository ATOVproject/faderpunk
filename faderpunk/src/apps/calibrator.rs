...
use crate::{
    ...
    tasks::{
        leds::LedMode,
        max::{MaxCalibration, MaxCmd, CALIBRATING, MAX_CHANNEL, MAX_VALUES_DAC, MAX_VALUES_FADER},
    },
};
...
pub async fn run(app: &App<CHANNELS>) {
    ...
    for (i, (&voltage, &target_value)) in VOLTAGES_0_10V.iter().zip(VALUES_0_10V.iter()).enumerate()
    {
        let pos = LED_POS[i];
    ...
    for (range_idx, &dac_range) in dac_ranges.iter().enumerate() {
        ...
        'channel_loop: while i < channels_to_calibrate.len() {
            ...
            if let Ok(results) = linear_regression::<f32, f32, f32>(
                ...
                calibration_data.outputs[chan][range_idx] = results;
                defmt::info!(
                    "Linear regression results for outputs channel {} range {}: {}",
                    chan,
                    range_idx,
                    calibration_data.outputs[chan][range_idx]
                );
            } else {
                // Blink LED red if calibration didn't succeeed
