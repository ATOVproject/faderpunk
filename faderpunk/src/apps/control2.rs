...
pub struct Params {
    curve: Curve,
    range: Range,
    midi_channel_fader: i32,
    midi_cc_fader: i32,
    midi_channel_button: i32,
    midi_cc_button: i32,
    color: Color,
    save_state: bool,
}
...
impl Default for Params {
    fn default() -> Self {
        Self {
            curve: Curve::Linear,
            range: Range::_0_10V,
            midi_channel_fader: 1,
            midi_cc_fader: 32,
            midi_channel_button: 1,
            midi_cc_button: 33,
            color: Color::Green,
            save_state: true,
        }
...
impl AppParams for Params {
    fn from_values(values: &[Value]) -> Option<Self> {
        ...
        Some(Self {
            ...
            midi_channel_fader: i32::from_value(values[2]),
            midi_cc_fader: i32::from_value(values[3]),
            midi_channel_button: i32::from_value(values[4]),
            midi_cc_button: i32::from_value(values[5]),
...
pub async fn run(
    ...
    let midi_fader = app.use_midi_output(midi_channel_fader as u8 - 1);
    let midi_button = app.use_midi_output(midi_channel_button as u8 - 1);
    ...
    let main_loop = async {
        ...
        loop {
            ...
            if last_out != (out as u32 * 127) / 4095 {
                midi_fader.send_cc(midi_cc_fader as u8, out).await;
            }
            jack.set_value(out);
        ...
        loop {
            ...
            if !buttons.is_shift_pressed() {
                if storage.query(|s| s.toggle) {
                    ...
                    midi_button
                        .send_cc(midi_cc_button as u8, button_state as u16 * 4095)
                        .await;
                    if button_state {
                        leds.set(0, Led::Button, led_color, Brightness::Lower);
                    } else {
                    ...
                    midi_button.send_cc(midi_cc_button as u8, 4095).await;
                    leds.set(0, Led::Button, led_color, Brightness::Lower);
                }
            } else {
    ...
    let button_up_handler = async {
        loop {
            ...
            if !storage.query(|s| s.toggle) {
                midi_button.send_cc(midi_cc_button as u8, 0).await;
                leds.unset(0, Led::Button);
            }
        }
    };
