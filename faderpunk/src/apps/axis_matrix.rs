...
pub async fn run(
    ...
    let fut_engine = async {
        ...
        loop {
            match select(
                app.delay_millis(2),
                clock.wait_for_event(ClockDivision::_1),
            )
            .await
            {
                Either::First(()) => {
                    ...
                    if center != prev_center || func != prev_func {
                        ...
                        let (quality, root_offset) = if in_transition {
                            ...
                            (
                                function_quality(func),
                                function_root_offset(center, func, interval_semi),
                            )
                        };
                ...
                Either::Second(ev) => match ev {
                    ...
                    ClockEvent::Tick(t) => {
                        ...
                        if t == 0 || !t.is_multiple_of(div) {
                            continue;
                        }

                        if glob_muted.get() || glob_latched.get() {
                            continue;
                        }
                        ...
                        let (center, in_transition) =
                            position_to_center_and_approach(pos_fader);
                        let func = function_from_fader(func_fader);
