...
use crate::{
    app::{App, colors::{WHITE, RED}, AppStorage, Led, ManagedStorage, Range, SceneEvent, RGB8},
    storage::ParamStore,
};
...
pub async fn run(app: &App<CHANNELS>, _params: &Params, storage: ManagedStorage<Storage>) {
    ...
    let fut1 = async {
        loop {
            ...
            if !buttons.is_shift_pressed() {
                ...
                leds.set(0, Led::Bottom, color, ((255.0 - (val as f32) / 16.0) / 2.0) as u8,);
            } else {
    ...
    let fut2 = async {
        loop {
            ...
            if !buttons.is_shift_pressed() {
                if !latched_glob.get().await && return_if_close(fader, stored_faders){
                ...
                if !latched_glob.get().await && return_if_close(fader, att_glob.get().await){
                    latched_glob.set(true).await;
                }
                if latched_glob.get().await {
