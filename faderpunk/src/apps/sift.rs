...
.add_param(Param::Enum {
    name: "Polarity",
    variants: &["Positive", "Bipolar", "Negative"],
})
.add_param(Param::i32 {
    name: "Min CV x0.1V",
    min: -50,
    ...
    let clock_handler = async {
        ...
        loop {
            match clock.wait_for_event(ClockDivision::_1).await {
                event @ (ClockEvent::Reset | ClockEvent::Start) => {
                    match event {
                        ClockEvent::Reset => tick_origin = None,
                        ClockEvent::Start if tick_origin.is_some() => {
                            tick_origin = last_tick.map(|tick: u32| tick.wrapping_add(1));
                        }
                        _ => {}
                    }
                    if note_on.get() {
                ...
                ClockEvent::Tick(ticks) => {
                    ...
                    let clkn = tick.wrapping_sub(*tick_origin.get_or_insert(tick));
